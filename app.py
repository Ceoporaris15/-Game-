...
def ai_logic():
    ...
    for _ in range(actions):
        ...
        if random.random() < 0.25 and p1["nuke_point"] > 30:
            p1["nuke_point"] = max(0, p1["nuke_point"] - 50); s["logs"].insert(0, "🕵️ DEUS工作: 核妨害")
            continue
        if s["wmd_charging"]:
            ...
            if random.random() < (0.7 if s["difficulty"] == "超大国" else 0.2):
                s["wmd_charging"] = True; s["logs"].insert(0, "🚨 DEUS: 核充填")
            else:
...
def player_step(cmd):
    ...
    if cmd == "EXPAND":
        ...
        s["logs"].insert(0, f"🛠 軍拡中")
    elif cmd == "DEFEND": p1["shield_active"] = True; s["logs"].insert(0, "🛡 防衛")
    elif cmd == "MARCH":
        ...
        if p1["military"] >= 20:
            ...
            p2["territory"] -= steal; p1["colony"] += steal; s["logs"].insert(0, "🚩 占領")
    elif cmd == "SPY":
        if random.random() < spy_success_base:
            if s["wmd_charging"]: s["wmd_charging"] = False; s["logs"].insert(0, "🕵️ 潜入成功")
...
if s["difficulty"] is None:
    ...
    if st.button("社会主義国"): s["faction"] = "社会主義国"; s["player_ap"] = 1; s["max_ap"] = 1; st.rerun()
else:
