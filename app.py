...
st.set_page_config(page_title="DEUS: March of Empire", layout="wide")
st.title("⚔️ 国家間Game：進軍の号令と占領の盾")
...
if 'state' not in st.session_state:
    st.session_state.state = {
        ...
        "turn": 1,
        "logs": ["SYSTEM: 進軍プロトコル。占領地が増えるほど進軍の威力が増大する。"],
        "wmd_charging": False,
        "ai_awakened": False 
    }
...
def apply_damage_to_player(dmg):
    if p1["colony"] > 0:
        ...
        dmg -= shield_amt
        if shield_amt > 0:
            s["logs"].insert(0, f"🛡️ 占領地が防波堤となり {shield_amt:.1f} の被害を吸収。")
...
def ai_logic_overload(player_last_action):
    ...
    for i in range(2):
        ...
        if p2["military"] < 100: p2["military"] = 100 # 即時チャージ

        if s["wmd_charging"]:
            if player_last_action == "MARCH" and random.random() < 0.4:
                s["logs"].insert(0, "✅ SYSTEM: 進軍による妨害でWMD停止！")
...
def player_step(cmd):
    if cmd == "MILITARY": 
        p1["military"] += 25.0
        p1["nuke_point"] += 15 
        s["logs"].insert(0, f"🔵 Player: チャージ完了（軍事:{p1['military']}）")
    elif cmd == "DEFEND": 
        p1["shield"] = True
        s["logs"].insert(0, "🔵 Player: 本国防衛。")
    elif cmd == "MARCH": # 攻撃から進軍へ
        # 占領地が多いほど攻撃力アップ
        dmg = (p1["military"] * 0.4) + (p1["colony"] * 0.5)
        if p2["shield"]: 
            p2["shield"] = False
            s["logs"].insert(0, "🔵 Player: 進軍（AIの防壁を粉砕）")
        else: 
            p2["territory"] -= dmg
            s["logs"].insert(0, f"🔵 Player: 進軍（AI領土に {dmg:.1f} の大ダメージ）")
    elif cmd == "OCCUPY":
        steal = min(p2["territory"] * 0.15, 35.0)
        p2["territory"] -= steal
        p1["colony"] += steal
        s["logs"].insert(0, f"🔵 Player: 占領。盾と兵站を確保（+{steal:.1f}）")
    elif cmd == "SPY":
    ...
    # 軍事力100で自動バースト
    if p1["military"] >= 100:
        burst_dmg = 80.0 + (p1["colony"] * 0.2)
        p2["territory"] -= burst_dmg
        p1["military"] = 0
        s["logs"].insert(0, f"💥 BURST!! 占領地と連携した総進軍で {burst_dmg:.1f} ダメージを与え、軍事力リセット。")
...
with col1:
    ...
    st.metric("占領地 (攻撃UP & 盾)", f"{p1['colony']:.1f}")
    st.write(f"軍事チャージ: {p1['military']}/100")
...
if p1["territory"] <= 0:
    ...
    st.success("【完全勝利】帝国は崩壊した。")
    if st.button("再試動"): st.session_state.clear(); st.rerun()
else:
    c = st.columns(5)
    if c[0].button("チャージ(1)"): player_step("MILITARY"); st.rerun()
