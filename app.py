...
st.markdown("""
    ...
    <script>
    // ブラウザの音響制限を解除するためのグローバル関数
    window.playTone = function(freq, type) {
        try {
...
# --- 2. 確定音響インジェクター ---
def inject_sound_trigger(sound_type):
    # 各ボタンが押された際、独立したHTMLコンポーネントとしてJavaScriptを実行
    freq = 440 if sound_type == "sharp" else 300
    wave = "square" if sound_type == "sharp" else "sine"
...
if 'state' not in st.session_state:
    st.session_state.state = {
        ...
        "turn": 1, "logs": ["システム起動。ボタンを押すと音響が同期されます。"],
...
def player_step(cmd):
    ...
    if cmd == "EXP":
        inject_sound_trigger("soft"); p1["military"] += 25.0 * a; p1["nuke_point"] += 20 * n
        s["logs"].insert(0, f"🛠軍拡: 軍備+{25.0*a:.0f}")
    elif cmd == "DEF":
        inject_sound_trigger("soft"); p1["shield"] = True; s["logs"].insert(0, "🛡防衛: シールド展開。")
    elif cmd == "MAR":
        inject_sound_trigger("sharp"); dmg = max(((p1["military"] * 0.5) + (p1["colony"] * 0.6)) * a + 10.0, 10.0)
        p2["territory"] -= dmg; s["logs"].insert(0, f"⚔️進軍: 敵へ{dmg:.0f}の打撃。")
    elif cmd == "OCC":
        inject_sound_trigger("soft"); steal = min(((max(p2["territory"] * 0.15, 25.0)) + 10.0) * o, 50.0)
        p1["colony"] += steal; s["logs"].insert(0, f"🚩占領: 緩衝地帯拡張。")
    elif cmd == "SPY":
        inject_sound_trigger("sharp")
        if random.random() < sp:
...
if s["phase"] == "DIFFICULTY":
    ...
    st.markdown('<div class="briefing-card"><div class="briefing-title">【アクション規定】</div><div class="briefing-text">'
                ...
                '・☢️<b>核兵器</b>: 開発200Pで使用可能。敵領土を85%破壊する。</div></div>', unsafe_allow_html=True)
    ...
    <div class="stat-section">
        <div class="stat-card">
            ...
            <div class="bar-label"><span>自国核兵器</span><span>{p1['nuke_point']:.0f}/200</span></div>
